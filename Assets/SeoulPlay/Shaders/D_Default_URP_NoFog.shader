...
Shader "Universal Render Pipeline/RealToon/Version 5/Default/Default No Fog"
{
    Properties
    {
		...
		[ToggleOff] _NorMapAsDis ("Normal Map As Displacement", Float ) = 0
...
//Tess_FP_2_End*/
		...
		) 
        {
			...
			#ifdef N_F_TP_ON
				half4 _MainTex_var = RT_Tripl_Default(_MainTex, sampler_MainTex, input.posWorld.xyz, normalDirection);
			#else
...
//DOTS_CompDef(input.vertexID, (float3)_LBS_CD_Position, _LBS_CD_Normal, (float3)_LBS_CD_Tangent);//DOTS_CompDef_SC
            ...
            output.projPos = ComputeScreenPos (output.positionCS);
			output.positionCS = GetShadowPositionHClip(input , output.positionWS.xyz , output.normalWS);
